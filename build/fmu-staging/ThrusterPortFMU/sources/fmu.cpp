...
// Custom made models
#include "../libraries/thrusterPortModel.hpp"



class ThrusterPortFMU : public cppfmu::SlaveInstance
{
public:
    // Constructor must be present
    ThrusterPortFMU()
    {
        ThrusterPortFMU::Reset();
    }
    ...
    void GetInteger(const cppfmu::FMIValueReference valueReferences[], std::size_t number_of_value_references, cppfmu::FMIInteger values[]) const override
    {
        for (std::size_t index = 0; index < number_of_value_references; ++index) {
            cppfmu::FMIValueReference valueReference = valueReferences[index];
            switch(valueReference) {
                // In this FMU we don't have integer data types, so all of these switches will just pass in case somehow our new vlaue in any of the varaibles didn't get updated in the last step in "SetReal()" method
                case 2:
        ...
        cppfmu::FMIReal& endOfTimeStep) override
    {
        ...
        // Simulation itself
        std::array<double, 2> results = ThrusterPortModel::simulate(
            timeStep,
            InputForce,
            InputAngle,
    ...
    cppfmu::FMIBoolean /*interactive*/, cppfmu::Memory memory, cppfmu::Logger /*logger*/)
{
    ...
    return cppfmu::AllocateUnique<ThrusterPortFMU>(memory);
}
