#include "thrusterBowModel.hpp"

namespace ThrusterBowModel {
    ...
    double forceRampUpDuration = 0.25;
    double angleRampUpDuration = 0.8;

    // Variable for simulating noise
    double forceNoiseMean = 0.0; // Newtons
    double forceNoiseStandardDeviation = 25.0; // Newtons
    double angleNoiseMean = 0.0; // Degrees
    double angleNoiseStandardDeviation = 0.25; // Degrees
    ...
    std::array<double, 2> simulate(double timeStep, double inputForce, double inputAngle, double parameterThrusterEfficiency, double currentForce, double currentAngle) {
        ...
        // Clamp the output angle to the valid range between (-90.0) and 90.0 degrees
        // This is because the real word Bow Thruster can only spin 270 Degrees 
        // However it is further limited to only 180 Degrees to reduce wear and tear, ie (-90.0) to 90.0 Degrees
        outputAngle = std::clamp(outputAngle, -90.0, 90.0);
