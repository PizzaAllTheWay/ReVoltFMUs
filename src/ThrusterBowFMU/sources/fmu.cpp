...
// Custom made models
#include "../libraries/thrusterBowModel.hpp"



class ThrusterBowFMU : public cppfmu::SlaveInstance
{
public:
    // Constructor must be present
    ThrusterBowFMU()
    {
        ThrusterBowFMU::Reset();
    }
    ...
    void Reset() override
    {
        ...
        ParameterThrusterEfficiency = 70; // 70 %
        ...
        cppfmu::FMIReal& endOfTimeStep) override
    {
        ...
        // Simulation itself
        std::array<double, 2> results = ThrusterBowModel::simulate(
            timeStep,
            InputForce,
            InputAngle,
    ...
    cppfmu::FMIBoolean /*interactive*/, cppfmu::Memory memory, cppfmu::Logger /*logger*/)
{
    ...
    return cppfmu::AllocateUnique<ThrusterBowFMU>(memory);
}
